...
import { followList } from '../../../../service/traderoom'
...
function TrustList(props) {
    ...
    const loadMoreOrder = () => {
        ...
        return followList(data).then((res) => {
            if (res.retCode === 0) {
    ...
    const navDetail = (item)=>{
        localStorage.setItem("strategyTrustDetailFollow",JSON.stringify(item))
        navigate('/strategyTrustDetailFollow/'+item.id)
    }
                ...
                return (
                    <div className="select-time-item" key={item.id}>
                        ...
                        <div className="info-wrapper" onClick={()=>navDetail(item)}>
                            ...
                            <Link className="signal-btn" to={'/strategyRealReport/' + item.id}>策略报告</Link>
                        </div>
                    </div>
                )
            })}

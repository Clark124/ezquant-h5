...
import { subscribeStrategy } from '../../../../service/strategy'


import './index.scss'
import arrow_icon from '../../../../asstes/images/grarrow.png'

import { changePeriod, setMyStrategyList, setStrategyIndex, setCurrentAccountId, setAccountList } from '../../../Trade/actions'
...
function StrategyTrust(props) {
    ...
    const getInit = () => {
        let params = localStorage.getItem("followParams")
        params = JSON.parse(params)
        let { period, symbol } = params
        let periodText = ""
        if (period) {
            if (period === 1) {
                periodText = '1分钟'
        ...
        Dialog.confirm({
            ...
            onConfirm: () => {
                ...
                const data = {
                    ...
                    symbol: backtestParameter.symbol,
                    ...
                    earlyNotice: 5,
                    otherStrategyId: backtestParameter.id
                }
                ...
                subscribeStrategy(data, head).then(res => {
                    Toast.clear()
                    if (res.code === 200) {
                        Toast.show({
                            icon: 'success',
    ...
    return (
        <div className="strategy-trust-wrapper">
            ...
            <div className="trust-item">
                <div>交易标的</div>
                <div className="trust-item-value">
                    <span>{backtestParameter.symbol}</span>
                </div>
            </div>
            <div className="trust-item">
                <div>策略资金</div>
                <input type="number" style={{ 'textAlign': 'center' }}
                    value={capital} placeholder="请输入托管资金" onChange={(e) => { setCapital(e.target.value) }} />
            </div>
            <div className="trust-item">
                <div>托管名称</div>
                <input type="text" placeholder="请输入托管名称" value={trustName} onChange={(e) => setTrustName(e.target.value)} />
            </div>

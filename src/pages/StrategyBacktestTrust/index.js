...
import { strategyList, deployStrategy, defaultAccount as defaultAccountFuc, accountList } from '../../service/traderoom'
...
function StrategyTrust(props) {
    ...
    useEffect(async () => {
        // const { account, accountList } = props
        // accountList.forEach(item => {
        //     if (item.tradeAccount === account.AccountID) {
    ...
    const getInit = () => {
        let id = params.id
        singleBacktestDetail({ fundKLineName: id }).then(res => {
            const { backtestParameter } = res.data
            ...
            if (backtestParameter.period) {
                if (backtestParameter.period === 1) {
                    ...
                    period = '1日'
                } else if (backtestParameter.period === 7) {
                    period = '1周'
                } else if (backtestParameter.period === 8) {
                    period = '1月'
                }
            }

            props.updateTrustData({ type: 'period', value: backtestParameter.period })
            props.updateTrustData({ type: 'periodText', value: period })
            props.updateTrustData({ type: 'capital', value: backtestParameter.initCapital })
        ...
        Dialog.confirm({
            ...
            onConfirm: () => {
                ...
                const data = {
                    ...
                    symbol: backtestParameter.prodCode,
                    hostingName: trustName,
                    description: discribe,
                    strategyName: props.myStrategyList[props.strategyIndex].name,
                    express: props.myStrategyList[props.strategyIndex].express,
                    strategyParams: props.myStrategyList[props.strategyIndex].params,
                ...
                if (positionIndex[positionValue] === 3) {
                    ...
                    if (Number(positionData.fixedPostionCount) % 100 !== 0 && props.marketType === 0) {
                        console.log(positionData.fixedPostionCount)
                        Modal.alert({
                ...
                console.log(data)
                const head = {
                    express: encodeURIComponent(props.myStrategyList[props.strategyIndex].express),
                    strategyParams: encodeURIComponent(props.myStrategyList[props.strategyIndex].params),
                }
                ...
                deployStrategy(data, head).then(res => {
                    Toast.clear()
                    if (res.code === 200) {
                        Toast.show({
                            icon: 'success',
    ...
    return (
        <div className="strategy-trust-wrapper">
            ...
            <div className="trust-item" onClick={() => navigate('/trade/myStrategyList')}>
                <div>托管策略</div>
                <div className="trust-item-value">
                    <span>{props.myStrategyList.length > 0 ? props.myStrategyList[props.strategyIndex].name : '--'}</span>
                    <img src={arrow_icon} className="arrow-icon" alt="" />
                </div>
            </div>
            ...
            <div className="trust-item">
                <div>交易标的</div>
                <div className="trust-item-value">
                    <span>{backtestParameter.prodName}({backtestParameter.prodCode})</span>
                </div>
            </div>
            <div className="trust-item">
                <div>策略资金</div>

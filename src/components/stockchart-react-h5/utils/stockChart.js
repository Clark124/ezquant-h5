...
export default class StockChart {
  constructor(canvas, data0, kBarBumber, ctx, option, quote, period) {
    this.maColor = {
      ma5: "#ff6767",
      ma10: "#4fce96",
      ma20: "#ffd202"
    ...
    this.maxValue = this.maxValue + (this.maxValue-this.minValue)*0.02
    this.minValue = this.minValue - (this.maxValue-this.minValue)*0.02
  ...
  initChart(isShowBoll, showCroll, info) {
    ...
    if (isShowBoll) {
      ...
      this.MA5 = this._calculateMA(5)
      this.MA10 = this._calculateMA(10)
      this.MA20 = this._calculateMA(20)
    }
    const ctx = this.ctx
    ...
    if (this.option.range && this.option.range.length > 1) {
      this._drawRange(this.option.range)
    }
  }
  //刷新K线
  refreshKline(value) {
    ...
    let newDate = value.categoryData[0]
    let oldDate = this.allData.categoryData[len - 1]

    const lastKValue = value.values[0]
    if (this.period < 6) {
      if(newDate.toString().length===3){
        newDate = "0"+newDate.toString()
      }
      newDate = Number(moment().format('YYYYMMDD') + newDate)
    }
   
    if (newDate === oldDate) {
      this.allData.values[len - 1] = lastKValue
      const lastKChangeRate = ((lastKValue[3] - this.allData.values[len - 2][3]) / this.allData.values[len - 2][3]) * 100
      this.allData.changeRate[len - 1] = lastKChangeRate
    }

    if (newDate > oldDate) {
      this.allData.values.push(value.values[0])
      this.allData.categoryData.push(newDate)
      this.allData.vol.push(value.vol[0])
      const lastKChangeRate = ((lastKValue[3] - this.allData.values[len - 1][3]) / this.allData.values[len - 1][3]) * 100
      this.allData.changeRate.push(lastKChangeRate)
      len = len + 1
    }
  ...
  _drawStockInfo(data) {
    ...
    if (this.period < 6) {
      ...
      date = date.substring(4, 6) + '-' + date.substring(6, 8) + ' ' + date.substring(8, 10) + ":" + date.substring(10, 12)
    } else {
  ...
  _drawRange(range) {
    let ctx = this.ctx
    let startDate = moment(range[0], 'YYYY-MM-DD').format('YYYYMMDD')
    let endDate = moment(range[1], 'YYYY-MM-DD').format('YYYYMMDD')
    ...
    if(!x2){
      x2 = this.canvas.width-this.cMargin
    }
    
    ctx.beginPath();

    ctx.moveTo(x1, this.originY);
    ctx.lineTo(x2, this.originY)
    ctx.lineTo(x2, 45)
    ctx.lineTo(x1, 45)
    ctx.closePath()
